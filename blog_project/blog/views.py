...
def global_setting(req):
    ...
    article_comment_list = [Article.objects.get(pk=comment['article']) for
                            comment in comment_count_list]
    return locals()
...
def archive(req):
    try:
        ...
        article_list = getPage(req, Article.objects.filter(
...
def article(req):
    try:
        ...
        comment_form = CommentForm({'author': req.user.username,
                                    ...
                                    'url': req.user.url,
                                    'article': id} if req.user.is_authenticated() else{
            'article': id})
        # 后去评论信息
